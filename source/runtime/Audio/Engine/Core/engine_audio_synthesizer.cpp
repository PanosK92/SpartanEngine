...
void Synthesizer::writeInput(const double *data) {
    ...
    for (int i = 0; i < m_inputChannelCount; ++i) {
        ...
        for (; s <= distance; s += 1.0) {
            ...
            buffer.write(static_cast<float>(
                m_filters[i].antialiasing.fast_f(
...
void Synthesizer::endInputBlock() {
    ...
    if (m_inputChannelCount != 0) {
        m_latency = static_cast<int>(
            m_inputChannels[0].data.size()
        );
    }
...
int16_t Synthesizer::renderAudio(int inputSample) {
    ...
    float signal = 0;
    for (int i = 0; i < m_inputChannelCount; ++i) {
        const float r_0 = static_cast<float>(
            2.0 * (
    ...
    int r_int = static_cast<int>(
        std::lround(v_leveled)
    );
    if (r_int > INT16_MAX) {
        r_int = INT16_MAX;
    }
