...
namespace car
{
    ...
    inline void integrate_tire_thermal(tire_thermal& thermal, const car_preset& preset,
        const float shares[3], float slip_power, float rolling_power, float airspeed, float dt, float ambient = NAN, float road = NAN, float contact = 0, float wet = 0)
    {
        if (!std::isfinite(ambient)) ambient = preset.tire_ambient_temp;
        if (!std::isfinite(road)) road = ambient;
        // a water film sits near the wet bulb temperature, conducts far better than air and the spray
        // off the tread evaporates, which is why wet tires struggle to reach their window
        const float water = PxClamp(wet, 0.0f, 1.0f);
        road += (ambient - 2.0f - road) * water;
        ...
        const float cooling = (preset.tire_heat_transfer_static + preset.tire_heat_transfer_airflow * fabsf(airspeed)) * (1.0f + 1.5f * water);
        ...
        for (int zone = 0; zone < 3; ++zone)
        {
            ...
            float surface_power = 0.9f * PxMax(slip_power, 0.0f) * shares[zone]
                ...
                + 40.0f * (1.0f + 3.0f * water) * PxClamp(contact, 0.0f, 1.0f) * shares[zone] * (road - thermal.surface[zone]);
            core_power += exchange;
