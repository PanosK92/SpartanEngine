...
static void ImGui_ImplSDL2_UpdateMouseCursor(Spartan::Context* context)
{
    ImGuiIO& io = ImGui::GetIO();
    ...
    // If ImGui is drawing the cursor, allow it to modify it's visibility
    if (io.MouseDrawCursor)
    {
        Spartan::Input* input = context->GetSubsystem<Spartan::Input>();
...
void ImGui_ImplSDL2_NewFrame(Spartan::Context* context)
{
    SDL_Window* window = static_cast<SDL_Window*>(context->GetSubsystem<Spartan::Window>()->GetHandleSDL());

    ImGuiIO& io = ImGui::GetIO();
    IM_ASSERT(io.Fonts->IsBuilt() && "Font atlas not built! It is generally built by the renderer backend. Missing call to renderer ImGui_ImplSDL2_NewFrame() function?");
    ...
    ImGui_ImplSDL2_UpdateMouseCursor(context);
