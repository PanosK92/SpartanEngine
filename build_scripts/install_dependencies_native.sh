...
# --- System Prep ---
# FreeImage include path compatibility (for <FreeImage/FreeImage.h>)
mkdir -p "$INSTALL_DIR/include/FreeImage"
ln -sf /usr/include/FreeImage.h "$INSTALL_DIR/include/FreeImage/FreeImage.h"
...
print_package_list() {
    local manager="$1"
    shift
    echo -e "${YELLOW}Install these packages manually with ${manager}:${NC}"
    echo "  $*"
}
...
install_packages() {
    ...
    if ! command -v "$manager" >/dev/null 2>&1; then
        echo -e "${RED}Package manager '$manager' is not available.${NC}"
        print_package_list "$manager" "$@"
        exit 1
    fi

    if [ "$(id -u)" -ne 0 ]; then
        echo -e "${RED}This script will not elevate privileges automatically.${NC}"
        print_package_list "$manager" "$@"
        echo -e "${YELLOW}Re-run as root to let the script install system packages.${NC}"
        exit 1
    fi
    ...
    if [ "$manager" = "dnf" ]; then
        ...
        echo -e "${RED}Unsupported package manager: $manager${NC}"
        exit 1
    fi
}
...
if [[ "$ID" =~ ^(fedora|nobara|rhel|centos)$ ]]; then
    PACKAGES=(
        gcc-c++ cmake ninja-build python3 wget git
        ...
        pulseaudio-libs-devel openssl-devel zlib-devel lua-devel openxr-devel
    )
    install_packages "dnf" "${PACKAGES[@]}"
elif [[ "$ID" == "arch" ]]; then
    PACKAGES=(
        gcc cmake ninja python wget git vulkan-headers freetype2
        pugixml freeimage mesa libx11 libxcursor libxext libxi libxinerama libxrandr
        libxss libxtst libxxf86vm wayland libxkbcommon pulseaudio openssl opencv zstd lua openxr
    )
    install_packages "pacman" "${PACKAGES[@]}"
elif [[ "$ID" =~ ^(ubuntu|debian)$ ]]; then
    PACKAGES=(
        g++ cmake ninja-build python3 wget git libvulkan-dev libfreetype-dev
        ...
        libpulse-dev libssl-dev zlib1g-dev libopencv-dev liblua5.4-dev libopenxr-dev
    )
    install_packages "apt" "${PACKAGES[@]}"
else
    echo -e "${RED}Unsupported distro: $ID${NC}"
    echo -e "${YELLOW}Please install required development packages manually and rerun.${NC}"
    exit 1
fi
...
compressonator_ready() {
    local split_core=0
    local monolithic_core=0

    if [ -f "$INSTALL_DIR/lib/libCMP_Core_SSE.a" ] && [ -f "$INSTALL_DIR/lib/libCMP_Core_AVX.a" ] && [ -f "$INSTALL_DIR/lib/libCMP_Core_AVX512.a" ]; then
        split_core=1
    fi

    if [ -f "$INSTALL_DIR/lib/libCMP_Core.a" ]; then
        monolithic_core=1
    fi

    [ -f "$INSTALL_DIR/lib/libCMP_Common.a" ] && \
    [ -f "$INSTALL_DIR/lib/libCMP_Framework.a" ] && \
    [ -f "$INSTALL_DIR/lib/libCMP_Compressonator.a" ] && \
    { [ $split_core -eq 1 ] || [ $monolithic_core -eq 1 ]; }
}
...
build_physx() {
    ...
    cmake --build . --config checked -j$(nproc) --target PhysXExtensions PhysXCommon PhysXPvdSDK PhysXCharacterKinematic PhysXCooking PhysXVehicle2 PhysX
...
build_nrd() {
    git_setup "nrd" "https://github.com/NVIDIA-RTX/NRD.git" "43fc6db11fd94097755c5e5cf5f95283a1acd2a9"
...
if compressonator_ready; then
    echo -e "${GREEN}✓ Compressonator already installed${NC}"
else
    echo -e "${BLUE}Building Compressonator...${NC}"
    cd "$BUILD_ROOT"
    build_compressonator
    echo -e "${GREEN}  ✓ Done${NC}"
fi
...
check "$INSTALL_DIR/lib/libCMP_Framework.a"
check "$INSTALL_DIR/lib/libCMP_Compressonator.a"
if [ -f "$INSTALL_DIR/lib/libCMP_Core.a" ]; then
    echo -e "${GREEN}✓ Found libCMP_Core.a${NC}"
else
    check "$INSTALL_DIR/lib/libCMP_Core_SSE.a"
    check "$INSTALL_DIR/lib/libCMP_Core_AVX.a"
    check "$INSTALL_DIR/lib/libCMP_Core_AVX512.a"
fi
