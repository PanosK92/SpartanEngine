...
const manifest=JSON.parse(fs.readFileSync('binaries/project/zakynthos_town/manifest.json','utf8'));
const c=new EngineClient({host:'127.0.0.1',port:47791,timeout_ms:60000,source:'town_authoring'});
try {
  for (const [i,m] of manifest.meshes.entries()) {
    ...
    if(i%10===0)console.log(`Town mesh ${i+1}/${manifest.meshes.length}`);
  }
  console.log('TOWN MESH IMPORT COMPLETE');
} finally {c.close();}
