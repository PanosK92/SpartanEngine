...
- A fixed 256 × 144 × 128 grid spans the camera to 32 km. The first 96 slices cover 64 metres; the first cell is about 7 cm deep. Moving across sea level never changes the grid mapping. Sky visibility is ray traced once per cell of a coarser 64 × 36 × 64 grid (`FOG_SKY_VISIBILITY`) and sampled trilinearly during injection.
...
- Injection does not read or clamp to opaque screen depth. Density is one filtered sample per cell at the grid centre (interface cells resample their air segment), with unresolved noise filtered to the cell footprint, avoiding object silhouettes and camera-dependent density oscillation. Raster consumers compensate for projection jitter before sampling the unjittered grid.
...
The renderer owns two air-light history textures, two water-light history textures, one RG32 material texture, separate RGB integrated scattering/transmission textures and one R8 coarse sky-visibility texture. Integrated textures include an identity boundary at the camera. This allocation uses **252.7 MiB**, independent of render resolution (the earlier 512 × 288 × 192 grid used 1514.25 MiB).
