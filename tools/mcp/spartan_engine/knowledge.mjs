...
export const engine_overview = [
  ...
  "spline_follower also animates car wheels through component_set: animate_wheels (bool) rolls every wheel with the travelled distance and steers the front wheels into the turns; it auto detects wheels from child entities whose name contains tire or wheel and splits front from rear by local position; wheel_radius (meters, 0 auto estimates from the wheel mesh bounds) controls roll speed and max_steer_angle (degrees) caps the front wheel steering; this runs while the sequencer plays or scrubs, no physics needed.",
].join("\n");
